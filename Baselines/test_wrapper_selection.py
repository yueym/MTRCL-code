...
def test_model(model, test_loader, device):
    ...
    # 加载wrapper标准化器
    scaler_target = joblib.load('./show_relevance_visualization_wrapper/target_scaler_wrapper.pkl')
    scaler_tigge = joblib.load('./show_relevance_visualization_wrapper/tigge_feature_scaler_wrapper.pkl')
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            all_tigge_wind.append(tigge_spatial[:, 7, :, :].cpu())
    ...
    # 保存结果文件，添加_wrapper后缀
    np.save('test_preds_wrapper.npy', all_preds_orig)
    np.save('test_targets_wrapper.npy', all_targets_orig)
    np.save('test_tigge_wind_wrapper.npy', all_tigge_wind_orig)
...
if __name__ == "__main__":
    ...
    # 加载测试数据集
    batch_size = 16
    test_ds = WindDatasetWrapper("./show_relevance_visualization_wrapper/test_wrapper.nc", H, W)
    ...
    # 加载训练好的模型
    model.load_state_dict(torch.load('checkpoints_wrapper/best_model_wrapper.pth', map_location=device))
    print("Wrapper model loaded successfully")

    # 开始测试
    test_model(model, test_loader, device)
    print("Wrapper method testing completed!")