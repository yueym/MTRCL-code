...
def train_model(model, train_loader, val_loader, device,
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'checkpoints/best_model_lstm.pth')
        ...
        torch.save({
            ...
            'val_rmse': val_rmse,
            'val_r2': val_r2
        }, f'checkpoints/checkpoint_epoch_{epoch + 1}_lstm.pth')
        epoch_end = time.time()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('mae_curve_lstm.png')
    plt.close()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('RMSE')
    plt.title('Root Mean Squared Error (LSTM)')
    plt.legend()
    plt.grid(True)
    plt.savefig('rmse_curve_lstm.png')
    plt.close()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('R²')
    plt.title('R² Score (LSTM)')
    plt.legend()
    plt.grid(True)
    plt.savefig('r2_curve_lstm.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] R² curve saved")

    print(
        f'[{time.strftime("%Y-%m-%d %H:%M:%S")}] Training metrics visualizations saved as: loss_curve_lstm.png, mae_curve_lstm.png, rmse_curve_lstm.png, r2_curve_lstm.png')
...
if __name__ == "__main__":
    ...
    batch_size = 8  # 减小batch size，LSTM内存占用较大
    epochs = 12
    learning_rate = 0.0001
    weight_decay = 0.001
    patience = 5
    accumulation_steps = 2  # 增加梯度累积步数补偿较小的batch size
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] LSTM model initialized successfully")
