...
class WindSpeedPredictor(nn.Module):
    ...
    def forward(self, tigge_spatial, dem_spatial):
        ...
        resnet_out = self.resnet(spatial_input)
...
def test_model(model, test_loader, device):
    ...
    with torch.no_grad():
        ...
        np.save('ResNet_test_preds.npy', all_preds_orig)
        np.save('ResNet_test_targets.npy', all_targets_orig)
        np.save('ResNet_test_tigge_wind.npy', all_tigge_wind_orig)
        np.save('ResNet_test_dates.npy', all_dates)
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Test predictions, targets, TIGGE wind, and dates saved as ResNet_test_*.npy files.")
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Yearly Test Metrics (ResNet):", metrics_all)
        ...
        for season, (start_month, end_month) in seasons.items():
            ...
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {season} Test Metrics (ResNet):",
                  seasonal_metrics[season])

        all_metrics = {'Yearly': metrics_all, **seasonal_metrics}
        with open('ResNet_test_metrics.json', 'w') as f:
            json.dump(all_metrics, f)
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Test metrics saved as ResNet_test_metrics.json")
        ...
        for month in range(1, 13):
            ...
            if np.sum(month_mask) > 0:
                ...
                print(
                    f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Month {months_names[month - 1]} Test Metrics (ResNet):",
                    monthly_metrics[month])

        monthly_metrics_dict = {str(k): v for k, v in monthly_metrics.items()}
        with open('ResNet_test_monthly_metrics.json', 'w') as f:
            json.dump(monthly_metrics_dict, f)
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Monthly test metrics saved as ResNet_test_monthly_metrics.json")
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('checkpoints/best_model_ResNet.pth'))
