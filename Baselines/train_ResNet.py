...
def set_seed(seed=None):
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Using random seed: {seed}")
...
def train_model(model, train_loader, val_loader, device,
                epochs=10, learning_rate=0.0001, weight_decay=0.001,
                patience=3, accumulation_steps=1):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'checkpoints/best_model_ResNet.pth')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('mae_curve_ResNet.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('rmse_curve_ResNet.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('r2_curve_ResNet.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] R² curve saved")

    print(f'[{time.strftime("%Y-%m-%d %H:%M:%S")}] Training metrics visualizations saved as: loss_curve_ResNet_Basic.png, mae_curve_ResNet_Basic.png, rmse_curve_ResNet_Basic.png, r2_curve_ResNet_Basic.png')
