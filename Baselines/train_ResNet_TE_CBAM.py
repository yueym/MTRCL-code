...
def train_model(model, train_loader, val_loader, device,
                epochs=10, learning_rate=0.0004, weight_decay=0.001,
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'checkpoints/best_model_ResNetCBAM_TE.pth')
    ...
    plt.savefig('loss_curve_ResNetCBAM_TE.png')
    plt.close()
    ...
    plt.savefig('mae_curve_ResNetCBAM_TE.png')
    plt.close()
    ...
    plt.savefig('rmse_curve_ResNetCBAM_TE.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('r2_curve_ResNetCBAM_TE.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] R² curve saved")

    print(
        f'[{time.strftime("%Y-%m-%d %H:%M:%S")}] Training metrics visualizations saved as: loss_curve_ResNetCBAM_TE.png, mae_curve_ResNetCBAM_TE.png, rmse_curve_ResNetCBAM_TE.png, r2_curve_ResNetCBAM_TE.png')
