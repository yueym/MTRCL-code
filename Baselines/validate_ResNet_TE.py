    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(val_loader):
            try:
                ...
                batch_dates = val_loader.dataset.time_points[start_idx:end_idx]
        ...
        np.save('ResNetTE_val_preds.npy', all_preds_orig)
        np.save('ResNetTE_val_targets.npy', all_targets_orig)
        np.save('ResNetTE_val_tigge_wind.npy', all_tigge_wind_orig)
        np.save('ResNetTE_val_dates.npy', all_dates)
        print("Validation predictions, targets, TIGGE wind, and dates saved as resnet_*.npy files.")
        ...
        print("Yearly Validation Metrics (ResNetTE):", metrics_all)
        ...
        for season, (start_month, end_month) in seasons.items():
            ...
            print(f"{season} Validation Metrics (ResNetTE):", seasonal_metrics[season])

        all_metrics = {'Yearly': metrics_all, **seasonal_metrics}
        with open('ResNetTE_val_metrics.json', 'w') as f:
            json.dump(all_metrics, f)
        print("Validation metrics saved as ResNetTE_val_metrics.json")
        ...
        for month in range(1, 13):
            ...
            if np.sum(month_mask) > 0:
                ...
                print(f"Month {months_names[month - 1]} Validation Metrics (ResNetTE):", monthly_metrics[month])

        monthly_metrics_dict = {str(k): v for k, v in monthly_metrics.items()}
        with open('ResNetTE_val_monthly_metrics.json', 'w') as f:
            json.dump(monthly_metrics_dict, f)
        print("Monthly validation metrics saved as ResNetTE_val_monthly_metrics.json")
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('checkpoints/best_model_ResNet_TE.pth'))  # 修改点 16：加载ResNet模型
