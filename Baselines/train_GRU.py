...
def train_model(model, train_loader, val_loader, device,
                epochs=12, learning_rate=0.0001, weight_decay=0.001,
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'checkpoints/best_model_gru.pth')  # GRU专用文件名
        ...
        torch.save({
            ...
            'val_rmse': val_rmse,
            'val_r2': val_r2
        }, f'checkpoints/checkpoint_epoch_{epoch + 1}_gru.pth')  # GRU专用文件名
        epoch_end = time.time()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('loss_curve_gru.png')  # GRU专用文件名
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('mae_curve_gru.png')  # GRU专用文件名
    plt.close()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('RMSE')
    plt.title('Root Mean Squared Error (GRU)')
    plt.legend()
    plt.grid(True)
    plt.savefig('rmse_curve_gru.png')  # GRU专用文件名
    plt.close()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('R²')
    plt.title('R² Score (GRU)')
    plt.legend()
    plt.grid(True)
    plt.savefig('r2_curve_gru.png')  # GRU专用文件名
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] R² curve saved")

    print(
        f'[{time.strftime("%Y-%m-%d %H:%M:%S")}] Training metrics visualizations saved as: loss_curve_gru.png, mae_curve_gru.png, rmse_curve_gru.png, r2_curve_gru.png')
...
if __name__ == "__main__":
    ...
    batch_size = 8  # 与LSTM保持一致的batch size
    epochs = 12
    learning_rate = 0.0001  # GRU通常也需要较小的学习率
    weight_decay = 0.001
    patience = 5
    accumulation_steps = 2  # 增加梯度累积步数
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] GRU model initialized successfully")
