...
# 数据集类 - 修改为使用wrapper数据集
class WindDatasetWrapper(Dataset):
    def __init__(self, ds_path, H=48, W=96, seq_len=4):
        self.H = H
        self.W = W
        self.seq_len = seq_len
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Loading WRAPPER dataset from {ds_path}")
        ...
        # 打印选择的特征信息
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Selected TIGGE features (RFE): {list(self.ds.coords['tigge_feature'].values)}")
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Selected DEM features (RFE): {list(self.ds.coords['dem_feature'].values)}")
        ...
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER dataset initialized with {len(self.sample_indices)} samples")
...
# 训练函数 - 修改保存文件名，添加_wrapper后缀
def train_model_wrapper(model, train_loader, val_loader, device,
                       epochs=12, learning_rate=0.0003931296040961602, weight_decay=0.0001025927948297095,
                       patience=5, accumulation_steps=1):
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Started WRAPPER method training process")
    ...
    # 创建带有_wrapper后缀的检查点目录
    checkpoint_dir = 'checkpoints_wrapper'
    os.makedirs(checkpoint_dir, exist_ok=True)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Wrapper checkpoint directory created: {checkpoint_dir}")
    ...
    for epoch in range(epochs):
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Starting epoch {epoch + 1}/{epochs} (WRAPPER method)")
        model.train()
        train_loss = 0.0
        batch_count = 0
        train_preds = []
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(checkpoint_dir, 'best_model_wrapper.pth'))
            print(f'[{time.strftime("%Y-%m-%d %H:%M:%S")}] WRAPPER model saved with val loss: {best_val_loss:.6f}')
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Saving WRAPPER checkpoint for epoch {epoch + 1}")
        torch.save({
            'epoch': epoch,
            ...
            'val_rmse': val_rmse,
            'val_r2': val_r2
        }, os.path.join(checkpoint_dir, f'checkpoint_epoch_{epoch + 1}_wrapper.pth'))
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER method training completed in {total_time / 3600:.2f} hours")

    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Generating WRAPPER visualization plots")

    # 所有图表文件名都添加 _wrapper 后缀
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training and Validation Loss (Wrapper Method)')
    plt.legend()
    plt.grid(True)
    plt.savefig('loss_curve_wrapper.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER loss curve saved")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('MAE')
    plt.title('Mean Absolute Error (Wrapper Method)')
    plt.legend()
    plt.grid(True)
    plt.savefig('mae_curve_wrapper.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER MAE curve saved")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('RMSE')
    plt.title('Root Mean Squared Error (Wrapper Method)')
    plt.legend()
    plt.grid(True)
    plt.savefig('rmse_curve_wrapper.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER RMSE curve saved")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('R²')
    plt.title('R² Score (Wrapper Method)')
    plt.legend()
    plt.grid(True)
    plt.savefig('r2_curve_wrapper.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER R² curve saved")

    print(f'[{time.strftime("%Y-%m-%d %H:%M:%S")}] WRAPPER training metrics visualizations saved as:')
    print('  - loss_curve_wrapper.png')
    print('  - mae_curve_wrapper.png')
    print('  - rmse_curve_wrapper.png')
    print('  - r2_curve_wrapper.png')
...
if __name__ == "__main__":
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER Method Training Program started")
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Setting up WRAPPER training parameters")
    batch_size = 16
    epochs = 12
    ...
    # 使用wrapper数据集路径
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Loading WRAPPER training dataset")
    train_ds = WindDatasetWrapper("./show_relevance_visualization_wrapper/train_wrapper.nc", H, W)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Loading WRAPPER validation dataset")
    val_ds = WindDatasetWrapper("./show_relevance_visualization_wrapper/val_wrapper.nc", H, W)

    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Creating WRAPPER data loaders")
    # 修改：Windows下使用 num_workers=0 避免多进程问题
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=False)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=False)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER data loaders created successfully")

    # 获取RFE选择后的特征数量
    tigge_features_count = len(train_ds.ds.coords['tigge_feature'])
    dem_features_count = len(train_ds.ds.coords['dem_feature'])
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER method using {tigge_features_count} TIGGE features and {dem_features_count} DEM features (selected by RFE)")

    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Initializing WRAPPER model")
    model = WindSpeedPredictor(
        H, W,
        tigge_features=tigge_features_count,  # 使用RFE选择后的特征数量
        dropout_rate=0.24733185479083603,
        ltc_hidden_dim=216,
        cbam_reduction=16
    ).to(device)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER model initialized successfully")

    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Starting WRAPPER Method Training")
    training_results = train_model_wrapper(
    ...
    # 保存训练结果到JSON文件，添加_wrapper后缀
    import json

    with open('training_results_wrapper.json', 'w') as f:
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER training results saved to training_results_wrapper.json")
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] WRAPPER Method Training completed!")

    # 打印最终结果摘要
    print(f"\n{'=' * 60}")
    print(f"WRAPPER METHOD TRAINING SUMMARY")
    print(f"{'=' * 60}")
    print(f"Feature Selection Method: RFE (Recursive Feature Elimination)")
    print(f"Base Estimator: LinearRegression")
    ...
    print(f"TIGGE Features Used (RFE): {tigge_features_count}")
    print(f"DEM Features Used (RFE): {dem_features_count}")
    ...
    print(f"Models:")
    print(f"  - checkpoints_wrapper/best_model_wrapper.pth")
    print(f"  - checkpoints_wrapper/checkpoint_epoch_*_wrapper.pth")
    print(f"Visualizations:")
    print(f"  - loss_curve_wrapper.png")
    print(f"  - mae_curve_wrapper.png")
    print(f"  - rmse_curve_wrapper.png")
    print(f"  - r2_curve_wrapper.png")
    print(f"Results:")
    print(f"  - training_results_wrapper.json")

    print(f"\nWrapper Method Details:")
    print(f"  ✅ RFE (Recursive Feature Elimination)")
    print(f"  📊 Linear Regression as base estimator")
    print(f"  🎯 Systematic feature elimination")
    print(f"  ⚡ Computationally efficient")
    print(f"  📈 Model-based feature importance")