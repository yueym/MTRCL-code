...
def validate_model(model, val_loader, device):
    ...
    all_tigge_wind = []
    all_dates = []
    criterion = nn.SmoothL1Loss()  # 无变化，与训练一致
    ...
    with torch.no_grad():
        ...
        np.save('LTCs_val_preds.npy', all_preds_orig)
        np.save('LTCs_val_targets.npy', all_targets_orig)
        np.save('LTCs_val_tigge_wind.npy', all_tigge_wind_orig)
        np.save('LTCs_val_dates.npy', all_dates)
        print("Validation predictions, targets, TIGGE wind, and dates saved as LTCs_*.npy files.")
        ...
        print("Yearly Validation Metrics (LTCs):", metrics_all)
        ...
        for season, (start_month, end_month) in seasons.items():
            ...
            print(f"{season} Validation Metrics (LTCs):", seasonal_metrics[season])

        all_metrics = {'Yearly': metrics_all, **seasonal_metrics}
        with open('LTCs_val_metrics.json', 'w') as f:
            json.dump(all_metrics, f)
        print("Validation metrics saved as LTCs_val_metrics.json")
        ...
        for month in range(1, 13):
            ...
            if np.sum(month_mask) > 0:
                ...
                print(f"Month {months_names[month - 1]} Validation Metrics (LTCs):", monthly_metrics[month])

        monthly_metrics_dict = {str(k): v for k, v in monthly_metrics.items()}
        with open('LTCs_val_monthly_metrics.json', 'w') as f:
            json.dump(monthly_metrics_dict, f)
        print("Monthly validation metrics saved as LTCs_val_monthly_metrics.json")
# 主函数
