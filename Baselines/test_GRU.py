...
def estimate_gru_flops(model, input_sample):
    """估算GRU模型的FLOPs"""
    ...
    print(f"Calculating FLOPs for GRU model...")
    print(f"Input shape - Batch: {batch_size}, Seq: {seq_len}, Channels: {tigge_channels}, H: {H}, W: {W}")

    # GRU部分的FLOPs计算
    print(f"Calculating GRU FLOPs...")

    # GRU输入维度计算
    gru_input_dim = tigge_channels * H * W + time_features * H * W
    gru_hidden_dim = 128
    gru_layers = 2

    # GRU的FLOPs计算 (每个时间步)
    # 对于每个GRU层: 3个门 × (input×hidden + hidden×hidden + bias)
    for layer in range(gru_layers):
        input_dim = gru_input_dim if layer == 0 else gru_hidden_dim

        # 输入到隐藏状态的变换: 3个门 × (input_dim × hidden_dim)
        # GRU有三个门: r(reset), z(update), n(new)
        input_to_hidden_flops = 3 * input_dim * gru_hidden_dim * 2  # *2 for multiply-add

        # 隐藏状态到隐藏状态的变换: 3个门 × (hidden_dim × hidden_dim)
        hidden_to_hidden_flops = 3 * gru_hidden_dim * gru_hidden_dim * 2
        ...
        print(f"  GRU Layer {layer + 1}: {layer_flops:,} FLOPs")
        print(f"    - Input dim: {input_dim}, Hidden dim: {gru_hidden_dim}")
...
class GRUFeatureExtractor(nn.Module):
    ...
    def forward(self, x_seq, time_features_seq):
        ...
        # GRU处理
        gru_out, hidden = self.gru(combined_features)

        # 使用最后一个时间步的输出
        final_output = gru_out[:, -1, :]  # (batch_size, hidden_dim)
...
class GatedFusion(nn.Module):
    def __init__(self, gru_dim, spatial_dim, output_dim):
        super(GatedFusion, self).__init__()

        # 确保维度匹配
        self.gru_proj = nn.Conv2d(gru_dim, output_dim, kernel_size=1)
    ...
    def forward(self, gru_out, spatial_out):
        # 投影到相同维度
        gru_proj = self.gru_proj(gru_out)
        spatial_proj = self.spatial_proj(spatial_out)

        # 计算门控权重
        combined = torch.cat([gru_proj, spatial_proj], dim=1)
        gate_weights = self.gate(combined)

        # 门控融合
        gated_gru = gate_weights * gru_proj
        gated_spatial = (1 - gate_weights) * spatial_proj

        # 最终融合
        final_combined = torch.cat([gated_gru, gated_spatial], dim=1)
...
class GRUWindSpeedPredictor(nn.Module):
    def __init__(self, H, W, tigge_features=8, dem_features=3, time_features=5,
                 gru_hidden_dim=128, spatial_hidden_dim=64, fusion_dim=64,
                 gru_layers=2, dropout_rate=0.4):
        super(GRUWindSpeedPredictor, self).__init__()
        self.H = H
        self.W = W

        # 计算GRU输入维度
        gru_input_dim = tigge_features * H * W + time_features * H * W
    ...
    def forward(self, tigge_spatial, dem_spatial, tigge_seq, time_features_t, time_features_seq):
        # GRU处理时序特征
        gru_features = self.gru_extractor(tigge_seq, time_features_seq)

        # 空间特征处理
        spatial_features = self.spatial_processor(tigge_spatial, dem_spatial)

        # 特征融合
        fused_features = self.gated_fusion(gru_features, spatial_features)
...
def test_model(model, test_loader, device):
    ...
    with torch.no_grad():
        ...
        # 保存GRU专用文件
        np.save('GRU_test_preds.npy', all_preds_orig)
        np.save('GRU_test_targets.npy', all_targets_orig)
        np.save('GRU_test_tigge_wind.npy', all_tigge_wind_orig)
        np.save('GRU_test_dates.npy', all_dates)
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Test predictions, targets, TIGGE wind, and dates saved as GRU_test_*.npy files.")
        ...
        # ===== 清晰输出7项测试指标 =====
        print(f"\n{'=' * 80}")
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] GRU MODEL - FINAL TEST RESULTS")
        print(f"{'=' * 80}")
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Yearly Test Metrics (GRU):", metrics_all)
        ...
        for season, (start_month, end_month) in seasons.items():
            ...
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {season} Test Metrics (GRU):",
                  seasonal_metrics[season])

        # 保存年度和季度指标
        all_metrics = {'Yearly': metrics_all, **seasonal_metrics}
        with open('GRU_test_metrics.json', 'w') as f:
            json.dump(all_metrics, f)
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Test metrics saved as GRU_test_metrics.json")
        ...
        for month in range(1, 13):
            ...
            if np.sum(month_mask) > 0:
                ...
                print(
                    f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Month {months_names[month - 1]} Test Metrics (GRU):",
                    monthly_metrics[month])

        # 保存月度指标
        monthly_metrics_dict = {str(k): v for k, v in monthly_metrics.items()}
        with open('GRU_test_monthly_metrics.json', 'w') as f:
            json.dump(monthly_metrics_dict, f)
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Monthly test metrics saved as GRU_test_monthly_metrics.json")
...
if __name__ == "__main__":
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Initializing GRU model...")
    ...
    # 加载GRU训练的最佳模型
    model.load_state_dict(torch.load('checkpoints/best_model_gru.pth'))
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] GRU model loaded successfully")
    ...
    total_flops = estimate_gru_flops(model, input_sample)
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Model: GRU")
