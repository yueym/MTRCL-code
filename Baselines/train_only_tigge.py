...
class ResNetCBAM(nn.Module):
    def __init__(self, in_channels=8, dropout_rate=0.25):
...
class ODEFunc(nn.Module):
    def __init__(self, hidden_dim=216, input_dim=8, dropout_rate=0.25):
...
class LTC(nn.Module):
    def __init__(self, input_dim=8, hidden_dim=216, output_dim=216, seq_len=4, dt=6.0, dropout_rate=0.25):
...
class WindSpeedPredictor_MeteoOnly(nn.Module):
    def __init__(self, H, W, tigge_features=8, dropout_rate=0.25, ltc_hidden_dim=216, cbam_reduction=16):
        super(WindSpeedPredictor_MeteoOnly, self).__init__()
        self.H = H
        self.W = W
        self.resnet = ResNetCBAM(in_channels=tigge_features, dropout_rate=dropout_rate)
        ...
        print(f"Total parameters (Meteo Only): {total_params:,}")

    def forward(self, tigge_spatial, tigge_seq, time_features_t, time_features_seq):
        b = tigge_spatial.size(0)
        time_emb = self.time_embed(time_features_t).view(b, 56, 1, 1)
        resnet_out = self.resnet(tigge_spatial, time_emb)
...
class WindDataset_MeteoOnly(Dataset):
...
def train_model(model, train_loader, val_loader, device, num_epochs=150):
    """训练模型"""
    print(f"开始训练模型 (只使用8个气象参数)...")

    # 创建保存目录
    os.makedirs('checkpoints_meteo', exist_ok=True)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            try:
                ...
                optimizer.zero_grad()
                output = model(tigge_spatial, tigge_seq, time_features_t, time_features_seq)
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), 'checkpoints_meteo/best_model_meteo_only.pth')
        ...
        # 每10轮保存一次模型
        if (epoch + 1) % 10 == 0:
            torch.save(model.state_dict(), f'checkpoints_meteo/model_epoch_{epoch + 1}_meteo_only.pth')
    ...
    with open('checkpoints_meteo/training_history_meteo_only.json', 'w') as f:
        json.dump(training_history, f, indent=2)

    print("训练完成！")
    print(f"最佳验证损失: {best_val_loss:.6f}")
    print("模型已保存至 checkpoints_meteo/best_model_meteo_only.pth")
...
def test_model(model, test_loader, device):
    ...
    # 保存测试结果
    with open('checkpoints_meteo/test_results_meteo_only.json', 'w') as f:
        json.dump({k: float(v) for k, v in test_metrics.items()}, f, indent=2)

    return test_metrics


# 添加主程序保护，解决Windows多进程问题
if __name__ == "__main__":
    ...
    train_ds = WindDataset_MeteoOnly("./show_relevance_visualization/train.nc", H, W)
    val_ds = WindDataset_MeteoOnly("./show_relevance_visualization/val.nc", H, W)
    test_ds = WindDataset_MeteoOnly("./show_relevance_visualization/test.nc", H, W)
    ...
    # 初始化模型
    print("初始化模型...")
    model = WindSpeedPredictor_MeteoOnly(
        H, W,
        tigge_features=8,
        dropout_rate=0.25,
        ltc_hidden_dim=216,
    ...
    # 测试模型
    print("开始测试...")
    model.load_state_dict(torch.load('checkpoints_meteo/best_model_meteo_only.pth'))
