...
class WindDatasetFilter(Dataset):
    def __init__(self, ds_path, H=48, W=96, seq_len=4):
        ...
        self.W = W
        self.seq_len = seq_len
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Loading FILTER dataset from {ds_path}")
        ...
        # 打印选择的特征信息
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Selected TIGGE features: {list(self.ds.coords['tigge_feature'].values)}")
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Selected DEM features: {list(self.ds.coords['dem_feature'].values)}")
        ...
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER dataset initialized with {len(self.sample_indices)} samples")
...
# 训练函数 - 修改保存文件名，添加_filter后缀
def train_model_filter(model, train_loader, val_loader, device,
                       epochs=12, learning_rate=0.0003931296040961602, weight_decay=0.0001025927948297095,
                       patience=5, accumulation_steps=1):
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Started FILTER method training process")
    ...
    # 创建带有_filter后缀的检查点目录
    checkpoint_dir = 'checkpoints_filter'
    os.makedirs(checkpoint_dir, exist_ok=True)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Filter checkpoint directory created: {checkpoint_dir}")
    ...
    for epoch in range(epochs):
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Starting epoch {epoch + 1}/{epochs} (FILTER method)")
        model.train()
        train_loss = 0.0
        batch_count = 0
        train_preds = []
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(checkpoint_dir, 'best_model_filter.pth'))
            print(f'[{time.strftime("%Y-%m-%d %H:%M:%S")}] FILTER model saved with val loss: {best_val_loss:.6f}')
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Saving FILTER checkpoint for epoch {epoch + 1}")
        torch.save({
            'epoch': epoch,
            ...
            'val_rmse': val_rmse,
            'val_r2': val_r2
        }, os.path.join(checkpoint_dir, f'checkpoint_epoch_{epoch + 1}_filter.pth'))
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER method training completed in {total_time / 3600:.2f} hours")

    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Generating FILTER visualization plots")

    # 所有图表文件名都添加 _filter 后缀
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training and Validation Loss (Filter Method)')
    plt.legend()
    plt.grid(True)
    plt.savefig('loss_curve_filter.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER loss curve saved")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('MAE')
    plt.title('Mean Absolute Error (Filter Method)')
    plt.legend()
    plt.grid(True)
    plt.savefig('mae_curve_filter.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER MAE curve saved")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('RMSE')
    plt.title('Root Mean Squared Error (Filter Method)')
    plt.legend()
    plt.grid(True)
    plt.savefig('rmse_curve_filter.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER RMSE curve saved")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('R²')
    plt.title('R² Score (Filter Method)')
    plt.legend()
    plt.grid(True)
    plt.savefig('r2_curve_filter.png')
    plt.close()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER R² curve saved")

    print(f'[{time.strftime("%Y-%m-%d %H:%M:%S")}] FILTER training metrics visualizations saved as:')
    print('  - loss_curve_filter.png')
    print('  - mae_curve_filter.png')
    print('  - rmse_curve_filter.png')
    print('  - r2_curve_filter.png')
...
if __name__ == "__main__":
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER Method Training Program started")
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Setting up FILTER training parameters")
    batch_size = 16
    epochs = 12
    ...
    # 使用过滤式数据集路径
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Loading FILTER training dataset")
    train_ds = WindDatasetFilter("./show_relevance_visualization_filter/train_filter.nc", H, W)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Loading FILTER validation dataset")
    val_ds = WindDatasetFilter("./show_relevance_visualization_filter/val_filter.nc", H, W)

    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Creating FILTER data loaders")
    # 修改：Windows下使用 num_workers=0 避免多进程问题
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=False)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=False)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER data loaders created successfully")

    # 获取过滤后的特征数量
    tigge_features_count = len(train_ds.ds.coords['tigge_feature'])
    dem_features_count = len(train_ds.ds.coords['dem_feature'])
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER method using {tigge_features_count} TIGGE features and {dem_features_count} DEM features")

    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Initializing FILTER model")
    model = WindSpeedPredictor(
        H, W,
        tigge_features=tigge_features_count,  # 使用过滤后的特征数量
        dropout_rate=0.24733185479083603,
        ltc_hidden_dim=216,
        cbam_reduction=16
    ).to(device)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER model initialized successfully")

    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Starting FILTER Method Training")
    training_results = train_model_filter(
    ...
    # 保存训练结果到JSON文件，添加_filter后缀
    import json

    with open('training_results_filter.json', 'w') as f:
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER training results saved to training_results_filter.json")
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FILTER Method Training completed!")

    # 打印最终结果摘要
    print(f"\n{'=' * 60}")
    print(f"FILTER METHOD TRAINING SUMMARY")
    print(f"{'=' * 60}")
    ...
    print(f"TIGGE Features Used: {tigge_features_count}")
    print(f"DEM Features Used: {dem_features_count}")
    print(f"{'=' * 60}")

    print(f"\nGenerated FILTER files:")
    print(f"Models:")
    print(f"  - checkpoints_filter/best_model_filter.pth")
    print(f"  - checkpoints_filter/checkpoint_epoch_*_filter.pth")
    print(f"Visualizations:")
    print(f"  - loss_curve_filter.png")
    print(f"  - mae_curve_filter.png")
    print(f"  - rmse_curve_filter.png")
    print(f"  - r2_curve_filter.png")
    print(f"Results:")
    print(f"  - training_results_filter.json")
