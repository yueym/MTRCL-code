...
def plot_spatial_metrics(spatial_metrics, lat_range=(35.13, 47), lon_range=(103, 126.88), save_dir='spatial_metrics',
    ...
    if os.path.exists(map_image_path):
        try:
            custom_map = plt.imread(map_image_path)
            print(f"成功加载自定义地图: {map_image_path}, 尺寸: {custom_map.shape}")
    ...
    for metric in metrics_display.keys():
        ...
        if custom_map is not None:
            # 在两个子图上都显示地图
            ax1.imshow(custom_map, extent=[lon_min, lon_max, lat_min, lat_max], aspect='auto', alpha=1.0,
                       origin='upper', zorder=1)
            ax2.imshow(custom_map, extent=[lon_min, lon_max, lat_min, lat_max], aspect='auto', alpha=1.0,
                       origin='upper', zorder=1)

            # 使用contourf绘制数据，设置适当的透明度
            masked_pred = np.ma.masked_invalid(pred_data)
            masked_tigge = np.ma.masked_invalid(tigge_data)

            sc1 = ax1.imshow(masked_pred,
                             extent=[lon_min, lon_max, lat_min, lat_max],
                             origin='upper',
                             aspect='auto',
                             cmap=cmap_dict[metric],
                             alpha=0.6,
                             vmin=vmin, vmax=vmax,
                             zorder=2)

            sc2 = ax2.imshow(masked_tigge,
                             extent=[lon_min, lon_max, lat_min, lat_max],
                             origin='upper',
                             aspect='auto',
                             cmap=cmap_dict[metric],
            ...
            # 如果没有自定义地图，使用scatter
            sc1 = ax1.scatter(lon_grid.flatten(), lat_grid.flatten(),
                              c=pred_data.flatten(), cmap=cmap_dict[metric],
                              s=10, alpha=0.8, vmin=vmin, vmax=vmax,
                              edgecolors='none')

            sc2 = ax2.scatter(lon_grid.flatten(), lat_grid.flatten(),
                              c=tigge_data.flatten(), cmap=cmap_dict[metric],
                              s=10, alpha=0.8, vmin=vmin, vmax=vmax,
                              edgecolors='none')
        ...
        if custom_map is None:
            for contour in contours:
                ...
                ax1.plot(contour_lons, contour_lats, 'k-', linewidth=0.8, alpha=0.7)
                ax2.plot(contour_lons, contour_lats, 'k-', linewidth=0.8, alpha=0.7)

        # 添加单一边框 - 修复双边框问题
        # 使用Rectangle对象代替plot边框
    ...
    for i, metric in enumerate(metrics_display.keys()):
        # 获取两个模型的数据
        pred_data = spatial_metrics[f'{metric}_pred']
        tigge_data = spatial_metrics[f'{metric}_tigge']
        ...
        if vmin is None or vmax is None:
            ...
            if len(combined_data) > 0:
                if vmin is None:
                    vmin = np.nanpercentile(combined_data, 1)
                if vmax is None:
                    vmax = np.nanpercentile(combined_data, 99)
        ...
        if custom_map is not None:
            # 在两个子图上都显示地图
            ax1.imshow(custom_map, extent=[lon_min, lon_max, lat_min, lat_max],
                       aspect='auto', alpha=1.0, origin='upper', zorder=1)
            ax2.imshow(custom_map, extent=[lon_min, lon_max, lat_min, lat_max],
                       aspect='auto', alpha=1.0, origin='upper', zorder=1)

            # 使用imshow代替contourf绘制数据
            masked_pred = np.ma.masked_invalid(pred_data)
            masked_tigge = np.ma.masked_invalid(tigge_data)
            ...
            sc1 = ax1.scatter(lon_grid.flatten(), lat_grid.flatten(),
                              c=pred_data.flatten(), cmap=cmap_dict[metric],
                              s=8, alpha=0.8, vmin=vmin, vmax=vmax,
                              edgecolors='none')
        ...
        ax1.set_title(f'Proposed: {metrics_display[metric]}', fontsize=12)
        ...
        hist_data1 = pred_data[~np.isnan(pred_data)].flatten()
        if len(hist_data1) > 0:
            hist_ax1.hist(hist_data1, bins=20, orientation='horizontal', color='blue', alpha=0.7)
            hist_ax1.set_ylim(vmin, vmax)
            hist_ax1.set_xticks([])
            hist_ax1.set_yticks([])
            hist_ax1.spines['top'].set_visible(False)
        ...
        ax2.set_title(f'ECMWF: {metrics_display[metric]}', fontsize=12)
        ...
        hist_data2 = tigge_data[~np.isnan(tigge_data)].flatten()
        if len(hist_data2) > 0:
            hist_ax2.hist(hist_data2, bins=20, orientation='horizontal', color='orange', alpha=0.7)
            hist_ax2.set_ylim(vmin, vmax)
            hist_ax2.set_xticks([])
            hist_ax2.set_yticks([])
            hist_ax2.spines['top'].set_visible(False)
        ...
        # 添加颜色条
        cbar_ax = fig.add_axes([0.92, 0.11 + i * 0.126, 0.02, 0.1])
        if custom_map is not None:
            cbar = fig.colorbar(sc1, cax=cbar_ax)
        else:
            cbar = fig.colorbar(sc1, cax=cbar_ax)
        cbar.set_label(metrics_display[metric], fontsize=10)

    # 保存组合图
    plt.suptitle('Spatial Distribution of Metrics: Proposed Model vs ECMWF', fontsize=20, y=0.98)
    plt.savefig(os.path.join(save_dir, 'test_spatial_metrics_combined.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def load_all_baseline_metrics():
    ...
    # 模型名称和对应的指标文件路径
    baseline_files = {
        'ConvLSTM': ('ConvLSTM_test_metrics.json', 'ConvLSTM_test_monthly_metrics.json'),
        'LTCs_TE': ('LTCs_test_metrics.json', 'LTCs_test_monthly_metrics.json'),
        'ResNet': ('ResNet_test_metrics.json', 'ResNet_test_monthly_metrics.json'),
        'ResNet_TE': ('ResNetTE_test_metrics.json', 'ResNetTE_test_monthly_metrics.json'),
        'ResNet_TE_CBAM': ('ResNetCBAMTE_test_metrics.json', 'ResNetCBAMTE_test_monthly_metrics.json')
    }
...
def plot_combined_time_analysis(all_preds, all_targets, all_tigge_wind, all_dates, save_dir='combined_analysis'):
    ...
    for row, hour in enumerate(hour_values):
        ...
        ax_scatter.text(0.95, 0.35, metrics_text_pred, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='right',
                        color='red')
        ax_scatter.text(0.95, 0.35 - spacing, metrics_text_pred_2, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='right',
                        color='red')
        ax_scatter.text(0.95, 0.35 - spacing * 2, metrics_text_pred_3, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='right',
                        color='red')
        ax_scatter.text(0.95, 0.35 - spacing * 3, metrics_text_pred_4, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='right',
                        color='red')
        ax_scatter.text(0.95, 0.35 - spacing * 4, metrics_text_pred_5, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='right',
                        color='red')
        ax_scatter.text(0.95, 0.35 - spacing * 5, metrics_text_pred_6, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='right',
                        color='red')

        # ECMWF-TIGGE指标放在左上角 - 增加垂直间距并使用调整后的指标值
        metrics_text_tigge = f"ECMWF-TIGGE: y={z_tigge:.2f}x"
        metrics_text_tigge_2 = f"N={len(hour_targets)}"
        metrics_text_tigge_3 = f"RMSE={metrics['RMSE_tigge']:.3f}"  
        metrics_text_tigge_4 = f"MAE={metrics['MAE_tigge']:.3f}"  
        metrics_text_tigge_5 = f"FA={metrics['FA_tigge']:.2f}%"  
        metrics_text_tigge_6 = f"R={metrics['R_tigge']:.3f}"  

        ax_scatter.text(0.05, 0.85, metrics_text_tigge, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='left',
                        color='blue')
        ax_scatter.text(0.05, 0.85 - spacing, metrics_text_tigge_2, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='left',
                        color='blue')
        ax_scatter.text(0.05, 0.85 - spacing * 2, metrics_text_tigge_3, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='left',
                        color='blue')
        ax_scatter.text(0.05, 0.85 - spacing * 3, metrics_text_tigge_4, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='left',
                        color='blue')
        ax_scatter.text(0.05, 0.85 - spacing * 4, metrics_text_tigge_5, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='left',
                        color='blue')
        ax_scatter.text(0.05, 0.85 - spacing * 5, metrics_text_tigge_6, transform=ax_scatter.transAxes,
                        fontsize=metrics_font_size, fontweight='bold', verticalalignment='top',
                        horizontalalignment='left',
                        color='blue')
...
def test_model(model, test_loader, device):
    ...
    with torch.no_grad():
        ...
        # 更新模型颜色映射
        model_colors = {
            'MTRCL': '#e3716e',  # Proposed -> MTRCL
            'TE-LTCs': '#eca680',  # LTCs_TE -> TE-LTCs
            'TE-ResNet': '#99b9e9',  # ResNet_TE -> TE-ResNet
            'TC-ResNet': '#f9d580',  # ResNet_TE_CBAM -> TC-ResNet
            'ECMWF-TIGGE': '#fccccb',  # ECMWF -> ECMWF-TIGGE
            'ResNet': '#b0d992',
            'ConvLSTM': '#bdb5e1'
        }
        ...
        # 创建微小偏移
        offset_value = 0.1  # 偏移量，可以根据数据范围调整
        offset_mtrcl = 0.2
        ...
        for i, hour in enumerate(hours):
            ...
            ax.plot(days_of_year, daily_preds + offset_mtrcl, label='MTRCL',
                    color=colors['MTRCL'], linewidth=1.5,zorder = 2)
            ax.plot(days_of_year, daily_tigge + offset_value , label='ECMWF-TIGGE',
                    color=colors['ECMWF'], linewidth=1.5,zorder = 1)
        ...
        # 调用组合分析函数
        plot_combined_time_analysis(all_preds_orig, all_targets_orig, all_tigge_wind_orig, all_dates,
...
def plot_spatial_metrics_paper_style(preds, targets, tigge_wind, lat_range, lon_range, save_path):

    # 获取网格尺寸
    H, W = preds.shape[1], preds.shape[2]
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Grid dimensions: H={H}, W={W}")

    # 计算空间指标
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 计算空间指标...")
    spatial_metrics = calculate_spatial_metrics(preds, targets, tigge_wind)

    metrics = ['FA', 'RMSE', 'MAE', 'rRMSE', 'rMAE', 'R']

    # 创建华北地区掩膜
    north_china_mask = create_north_china_mask(H, W, lat_range[0], lat_range[1], lon_range[0], lon_range[1])
    ...
    for metric in metrics:
        for model in ['pred', 'tigge']:
            key = f'{metric}_{model}'
            if key in spatial_metrics:
                data = spatial_metrics[key]
                spatial_metrics[key] = np.where(north_china_mask, data, np.nan)
    ...
    vmin_dict = {'FA': 0, 'RMSE': None, 'MAE': None, 'rRMSE': 0, 'rMAE': 0, 'R': -1}
    vmax_dict = {'FA': 100, 'RMSE': None, 'MAE': None, 'rRMSE': 100, 'rMAE': 100, 'R': 1}
    metric_name_map = {
        'FA': 'FA (%)',
        'RMSE': 'RMSE (m/s)',
        'MAE': 'MAE (m/s)',
    ...
    lat_min, lat_max = lat_range
    lon_min, lon_max = lon_range
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Latitude range: {lat_min} to {lat_max}")
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Longitude range: {lon_min} to {lon_max}")

    # 创建经纬度网格
    lats = np.linspace(lat_min, lat_max, H)
    lons = np.linspace(lon_min, lon_max, W)

    # 创建边界网格 - 类似于 plot_china_wind_distribution 的方法
    lons_edges = np.linspace(lon_min, lon_max, W + 1)
    lats_edges = np.linspace(lat_min, lat_max, H + 1)

    # 创建图形 - 改为6行2列
    fig, axes = plt.subplots(len(metrics), 2, figsize=(20, 24))
    plt.subplots_adjust(left=0.1, right=0.9, bottom=0.05, top=0.95, wspace=0.2, hspace=0.3)

    # 设置全局字体属性
    plt.rcParams['font.weight'] = 'bold'
    font_size = 14  # 增大字体大小，从12改为14

    # 模型名称 - 修改模型名称
    model_names = ['MTRCL', 'ECMWF-TIGGE']
    model_keys = ['pred', 'tigge']
    ...
    try:
        ...
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 无法加载地图图像: {str(e)}")
        has_map = False
    ...
    for i, metric in enumerate(metrics):
        ...
        for model_key in model_keys:
            key = f'{metric}_{model_key}'
            # 使用调整后的指标值绘制ECMWF-TIGGE
            data = spatial_metrics[key] if model_key == 'tigge' else spatial_metrics[key]
        ...
        vmin = vmin_dict[metric]
        vmax = vmax_dict[metric]
        if vmin is None and len(all_data) > 0:
            vmin = np.min(all_data)
        if vmax is None and len(all_data) > 0:
            vmax = np.max(all_data)
        print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Metric {metric}: vmin={vmin}, vmax={vmax}")

        for j, (model_name, model_key) in enumerate(zip(model_names, model_keys)):
            ax = axes[i, j]
            ax.clear()
            ...
            # 使用调整后的指标值绘制ECMWF-TIGGE
            data = spatial_metrics[key] if model_key == 'tigge' else spatial_metrics[key]
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Plotting {key}, data shape: {data.shape}")

            # 确保数据形状正确
            data_filled = np.copy(data)
            if data_filled.shape != (H, W):
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Reshaping {key} from {data_filled.shape} to ({H}, {W})")
                data_filled = data_filled.reshape(H, W) if data_filled.size == H * W else np.full((H, W), np.nan)

            # 插值处理 NaN
            valid_mask = ~np.isnan(data_filled)
            if np.any(valid_mask):
                y_indices, x_indices = np.mgrid[0:H, 0:W]
                valid_points = np.column_stack((y_indices[valid_mask], x_indices[valid_mask]))
                valid_values = data_filled[valid_mask]
                interp = NearestNDInterpolator(valid_points, valid_values)
                all_points = np.column_stack((y_indices.ravel(), x_indices.ravel()))
                data_filled = interp(all_points).reshape(H, W)
            else:
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Warning: No valid data for {key}, filling with NaN")
                data_filled = np.full((H, W), np.nan)

            data_filled = np.clip(data_filled, vmin, vmax)

            # 创建主图区域
            ax.set_frame_on(True)
            for spine in ax.spines.values():
                spine.set_visible(True)
                spine.set_linewidth(1.5)  # 增加主图边框线宽，与直方图保持一致
            ...
            # 1. 首先绘制数据层
            im = ax.pcolormesh(lons_edges, lats_edges, data_filled,
                               cmap=cmap_dict[metric], vmin=vmin, vmax=vmax,
                               alpha=0.75, zorder=2, shading='flat')  # 略微降低透明度以便更好地显示边界线

            # 2. 如果有地图，叠加线条图
            if has_map:
                # 使用imshow绘制线条图 - 确保数据范围在[0,1]内
                ax.imshow(lines_map, extent=[lon_min, lon_max, lat_min, lat_max],
                          aspect='auto', origin='upper', zorder=3, interpolation='nearest')
            ...
            # 计算子图索引对应的字母标记 (a, b, c, ...)
            subplot_index = i * 2 + j
            subplot_letter = chr(97 + subplot_index)  # 97是字母'a'的ASCII码

            # 移除原有标题
            ax.set_title("")

            # 在左上角添加带字母标记的标题
            ax.text(0.01, 1.02, f"({subplot_letter}) {metric_name_map[metric]} - {model_name}",
            ...
            # 设置颜色条标签并加粗 - 统一并增大字体大小
            if metric in ['FA', 'rRMSE', 'rMAE']:
                cbar.set_label('%', fontsize=font_size, fontweight='bold')
            elif metric in ['RMSE', 'MAE']:
                cbar.set_label('m/s', fontsize=font_size, fontweight='bold')
            # 对于 R 指标不显示单位

            # 设置颜色条刻度标签字体 - 明确设置字体大小和粗细
            ...
            # 设置经度刻度 - 从103到126，选取几个整数刻度
            lon_ticks = [103, 110, 115, 120, 126]  # 手动指定整数刻度
            ax.set_xticks(lon_ticks)
            lon_labels = [f"{int(x)}°E" for x in lon_ticks]
            ax.set_xticklabels(lon_labels)

            # 设置纬度刻度 - 使用整数刻度
            lat_start = int(np.ceil(lat_min))  # 向上取整到最近的整数
            lat_end = int(np.floor(lat_max))  # 向下取整到最近的整数
            lat_ticks = np.arange(lat_start, lat_end + 1, 3)  # 每5度一个刻度
            ax.set_yticks(lat_ticks)
            lat_labels = [f"{int(y)}°N" for y in lat_ticks]
            ax.set_yticklabels(lat_labels)
            ...
            # 绘制直方图（如果需要）- 增加左侧间距以避免与主图刻度重叠
            valid_data = data[~np.isnan(data)]
            if len(valid_data) > 10:
                # 增加pad值，使直方图向左移动更多，避免与主图坐标轴重叠
                hist_ax = divider.append_axes("left", size="20%", pad=0.8)  # 增加pad值从0.7到0.8

                for spine in hist_ax.spines.values():
                    spine.set_linewidth(1.5)  # 设置直方图边框线宽与主图一致

                hist_bins = np.linspace(0, 100, 11) if metric in ['FA', 'rRMSE', 'rMAE'] else np.linspace(vmin, vmax,
                                                                                                          11)
                hist, bins = np.histogram(valid_data, bins=hist_bins)
                width = 0.85 * (bins[1] - bins[0])
                center = (bins[:-1] + bins[1:]) / 2
                hist_ax.barh(center, hist, height=width, color='gray', alpha=0.8)
                hist_ax.set_ylim(vmin, vmax)

                # 统一直方图标签字体
                hist_ax.set_xlabel('Number of samples', fontsize=font_size, fontweight='bold')
                hist_ax.set_ylabel(metric_name_map[metric], fontsize=font_size, fontweight='bold')
                ...
                # 可选：添加x10^2标题，并加粗并统一字体大小
                if np.max(hist) > 100:
                    hist_ax.set_title('1e2', fontsize=font_size, loc='right', fontweight='bold')
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] 空间指标比较图已保存至: {save_path}")
    plt.close()
